...
To help limit the number of events that are logged to your trail, you can filter out AWS KMS events by choosing **Exclude AWS KMS events** on the **Create trail** or **Update trail** pages\. The option to filter out AWS KMS events is only available if your trail is logging management events\. For more information, see [Creating a trail](cloudtrail-create-a-trail-using-the-console-first-time.md) or [Updating a trail](cloudtrail-update-a-trail-console.md) in this guide\.
...
## See Also<a name="w215aac10c30b7b9"></a>
