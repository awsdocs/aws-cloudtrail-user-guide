...
1. For **Data events**, you can specify logging data events for Amazon S3 buckets, AWS Lambda functions, Amazon DynamoDB tables, or a combination of these resource types\. By default, trails don't log data events\. Additional charges apply for logging data events\. For more information, see [Data events](logging-data-events-with-cloudtrail.md#logging-data-events)\. For CloudTrail pricing, see [AWS CloudTrail Pricing](https://aws.amazon.com/cloudtrail/pricing/)\. Additional data event types are available if you use advanced event selectors; for more information, see [Creating a trail in the console \(advanced event selectors\)](#creating-a-trail-in-the-console-adv) in this topic\.
...
1. For **Data event type**, choose the data event resource type \(S3 buckets, S3 objects on AWS Outposts, Amazon Managed Blockchain nodes, Lambda functions, DynamoDBtables, or S3 Object Lambda access points\) that you want to log\.
...
1. In **Advanced event selectors**, build an expression for the specific S3 buckets, S3 objects on AWS Outposts, Lambda functions, or DynamoDB tables on which you want to log data events\.
   ...
   1. Choose from the following fields\. For fields that accept an array \(more than one value\), CloudTrail adds an OR between values\.
      ...
      + **`eventName`** \- `eventName` can use any operator\. You can use it to include or exclude any data event logged to CloudTrail, such as `PutBucket` or `PutItem`\. You can have multiple values for this field, separated by commas\.
